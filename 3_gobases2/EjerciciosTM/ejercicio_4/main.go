package main

import "fmt"

const (
	suma     = "+"
	resta    = "-"
	multip   = "*"
	divis    = "/"
	minimo   = "minimo"
	promedio = "promedio"
	maximo   = "maximo"
)

func main() {
	fmt.Println(operacionAritmetica(suma, 2, 3, 2, 1, 2, 3, 4, 5, 6))
	fmt.Println(operacionAritmetica(resta, 2, 3, 2, 1, 2, 3, 4, 5, 6))
	fmt.Println(operacionAritmetica(multip, 2, 3, 2, 1, 2, 3, 4, 5, 6))
	fmt.Println(operacionAritmetica(divis, 2, 3, 2, 1, 2, 3, 4, 5, 6))
	fmt.Println(operacionAritmetica(minimo, 2, 3, 2, 1, 2, 3, 4, 5, 6))
	fmt.Println(operacionAritmetica(promedio, 2, 3, 2, 1, 2, 3, 4, 5, 6))
	fmt.Println(operacionAritmetica(maximo, 2, 3, 2, 1, 2, 3, 4, 5, 6))
}

func operacionAritmetica(operador string, valores ...float64) float64 {
	switch operador {
	case suma:
		return orquestadorOperaciones(valores, opSuma)
	case resta:
		return orquestadorOperaciones(valores, opResta)
	case multip:
		return orquestadorOperaciones(valores, opMultip)
	case divis:
		return orquestadorOperaciones(valores, opDivis)
	case minimo:
		return orquestadorOperacionesDos(valores, opMinimo)
	case promedio:
		return orquestadorOperacionesDos(valores, opPromedio)
	case maximo:
		return orquestadorOperacionesDos(valores, opMaximo)
	}
	return 0
}

func orquestadorOperaciones(valores []float64, operacion func(value1, value2 float64) float64) float64 {
	var resultado float64
	for i, valor := range valores {
		if i == 0 {
			resultado = valor
		} else {
			resultado = operacion(resultado, valor)
		}
	}
	return resultado
}

func orquestadorOperacionesDos(valores []float64, operacion func(numeros []float64) float64) float64 {
	resultado := operacion(valores)
	return resultado
}

func opSuma(valor1, valor2 float64) float64 {
	return valor1 + valor2
}

func opResta(valor1, valor2 float64) float64 {
	return valor1 - valor2
}

func opMultip(valor1, valor2 float64) float64 {
	return valor1 * valor2
}

func opDivis(valor1, valor2 float64) float64 {
	if valor2 == 0 {
		return 0
	}
	return valor1 / valor2
}

func opMinimo(valores []float64) float64 {
	var min float64 = valores[0]

	for _, value := range valores {
		if min > value {
			min = value
		}
	}

	return min
}

func opPromedio(valores []float64) float64 {
	var sum float64

	for _, v := range valores {
		sum += v
	}

	return sum / float64(len(valores))
}

func opMaximo(valores []float64) float64 {
	var max float64 = valores[0]

	for _, value := range valores {
		if max < value {
			max = value
		}
	}

	return max
}
