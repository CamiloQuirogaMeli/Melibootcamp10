...
func transactionsPost() gin.HandlerFunc {
	return func(c *gin.Context) {
		var req Transaction
		//Validate all position Transaction struct fields
		// var errorsBind map[string]
		var errorsBind2 = []string{}
		if err := c.ShouldBindJSON(&req); err != nil {
			for _, err := range err.(validator.ValidationErrors) {
				mensaje := fmt.Sprintf("el campo %s es requerido", err.Field())
				// errorsBind[err.Field()] = mensaje
				errorsBind2 = append(errorsBind2, mensaje)
			}
			// c.JSON(400, gin.H{
			// 	"error": err.Error(),
			// })
			c.JSON(400, errorsBind2)
			return
		} else {
			maxId++
			req.Id = int64(maxId)
