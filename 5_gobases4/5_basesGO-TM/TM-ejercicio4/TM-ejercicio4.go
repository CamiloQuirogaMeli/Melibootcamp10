...
func salarioMensual(minHoras int, hsTrabajadas int, valorHora float64) (float64, error) {

	if hsTrabajadas < minHoras {
		return 0, fmt.Errorf("error: el trabajador no puede haber trabajado menos de %d hs mensuales", minHoras)
	}
...
func medioAguinaldo(mejorSalario float64, mesesTrabajados int) (float64, error) {
	...
	if mesesTrabajados < 0 {
		return 0, errors.New("error: la cantidad de meses trabajados no puede ser negativo")
	}
...
func main() {
	...
	salario, err1 := salarioMensual(80, 200, 800.0)
